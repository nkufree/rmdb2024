...
在本任务中，参赛队伍需要实现缓冲池管理器BufferPoolManager和缓冲池替换策略Replacer相关的接口，
缓冲池管理器负责管理缓冲池中的页面在内外存的交换，缓冲池替换策略主要负责缓冲区页面的淘汰和查找。
在完成本任务之前，参赛队伍需要首先阅读项目结构文档中缓冲池管理器的相关说明，以及代码框架中src/storage和src/replacer文件夹下的代码文件
*/
...
private:
    ...
    Page *pages_;                                                   // buffer_pool中连续存放的的Page页面对象数组，在构造空间中申请内存空间，在析构函数中释
                                                                    // 放，大小为BUFFER_POOL_SIZE
    std::unordered_map<PageId, frame_id_t, PageIdHash> page_table_; // 帧号和页面号的映射哈希表，用于根据页面的PageId定位该页面的帧编号
                                                                    // ⻚表，记录数据⻚和帧的映射关系，如果数据⻚存放在某⼀帧中，则在⻚表中存放该数据⻚ID和对
                                                                    // 应帧号的映射关系，⽤于标记数据⻚在缓冲池中的存放位置
