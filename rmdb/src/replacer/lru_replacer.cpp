...
// 选择并淘汰缓冲池中一个页面。如果成功找到要淘汰的页面，则函数返回值为true；否则，返回值为false。被淘汰的页面所在的帧由参数frame_id返回。
/**
...
bool LRUReplacer::victim(frame_id_t *frame_id)
{
    ...
    if (LRUlist_.empty())
    {
        *frame_id = -1; // 如果没有可淘汰的页面，返回 nullptr
        return false;
    }
...
// 固定住一个帧中的页面，代表该页面正在使用，不可被换出，参数frame_id指定了帧的编号。
/**
...
// 取消固定一个帧中的页面，当该页面使用完毕时调用unpin函数取消对该页面的固定，参数frame_id指定了帧的编号。
/**
...
void LRUReplacer::unpin(frame_id_t frame_id)
{
    ...
    //  选择一个frame取消固定
    std::scoped_lock lock{latch_};
    if (LRUhash_.find(frame_id) != LRUhash_.end()) // 避免重复添加
    {
        return;
    }
